  ...
  return (
    <OuterBackground>
      <AppContainer>
        <Header>
          <HeaderLeft>
             <Logo onClick={() => navigate('/')} $isDarkMode={isDarkMode}>
              AZ News Azit
            </Logo>
          </HeaderLeft>
          ...
          <HeaderRight>
            ...
            <NotiWrapper>
              ...
              {isNotiOpen && (
                <NotiDropdown>
                  <NotiHeader>
                    <span>알림 센터</span>
                    <CloseBtn onClick={() => setIsNotiOpen(false)}><FaTimes /></CloseBtn>
                  </NotiHeader>
                  <NotiList>
            ...
            {user ? (
              <>
                {/* ★ 여기가 핵심 수정 포인트! (닉네임 우선) */}
                <UserName>{user.nickname || user.name || "회원"}님</UserName>
                <IconButton onClick={logout} title="로그아웃">
        ...
        <BodyWrapper>
          <NavBar>
            <NavItem to="/" end>
              <FaRegNewspaper size={20} /><span>뉴스</span>
            </NavItem>
            <NavItem to="/board">
              <FaListAlt size={20} /><span>게시판</span>
            </NavItem>
            <NavItem to="/feed">
              <FaHashtag size={20} /><span>피드</span>
            </NavItem>
            <NavItem to="/mypage">
              <FaUser size={20} /><span>마이</span>
            </NavItem>
          </NavBar>
...
// --- [스타일 정의 (기존과 동일)] ---
const OuterBackground = styled.div` width: 100%; min-height: 100vh; background-color: #000000; display: flex; justify-content: center; `;
...
const CloseBtn = styled.button` background: none; border: none; color: ${props => props.theme.subText}; cursor: pointer; `;
...
const MainContent = styled.main` flex: 1; background-color: ${props => props.theme.bg}; padding: 20px; padding-bottom: 90px; width: 100%; @media (min-width: 768px) { margin-left: 240px; padding-bottom: 20px; padding-left: 40px; padding-right: 40px; } `;